...
class LeadspottingPostsImporter(CsvImporter):
    ...
    def create_post_dict_from_row(self, row):
        ...
        author_guid = unicode(compute_author_guid_by_author_name(row["name"]).replace('-', ''))
        ...
        post_dict["url"] = unicode("https://twitter.com/{0}/status/{1}".format(post_dict["author"], row["userId"]))
        return post_dict